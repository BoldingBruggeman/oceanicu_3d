#!/usr/bin/env python
"""experiment_tracking_server.py -- RPC entrypoint for remote registry access.

Deployed once to the relay machine (alongside experiment_tracking.py --
both files, nothing else needed) at whatever directory OCEANICU_RELAY_DIR
points callers at. Invoked over SSH, once per call, by RemoteConn.call()
in experiment_tracking.py -- never run directly by a human.

Reads ONE JSON request from stdin:
    {"db": "/abs/path/to/registry.sqlite", "func": "name", "kwargs": {...}}
opens that DB locally (real sqlite3, experiment_tracking.connect()'s normal
path -- this file only ever runs ON the relay, so "locally" here really
does mean the relay's own filesystem), calls experiment_tracking.<func>(conn,
**kwargs), and writes ONE JSON response line to stdout:
    {"ok": true, "result": ...}
    {"ok": false, "error": "..."}

*func* must name a function in experiment_tracking's public API -- arbitrary
...
# is_paused/next_experiment_to_start). getattr() alone would happily resolve
