...
case "$role" in
    hpc)
        ...
        echo "calls sbatch, for any run, new or resubmitting:"
        echo "  crontab -e   # on the login node -- then add a line like:"
        echo "  0 * * * * cd $scripts_dir && OCEANICU_RUN_DB=$path/run_registry.sqlite OCEANICU_RUN_ROOT_BASE=$path python3 get_commands_and_update_registry.py --db $path/run_registry.sqlite --queue-dir $path/hpc_commands --pull-from bb-server1:/data/OceanICU/oceanicu_3d/experiments/hpc_commands >> $path/hpc_commands/get_commands_and_update_registry.log 2>&1"
