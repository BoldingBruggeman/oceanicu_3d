...
Handles multiple files per variable with wildcard support on different spherical grids.
"""
...
class MultiVariableAnalyzer:
    ...
    Supports multiple files per variable with wildcard patterns.
    """
    ...
    def regrid_observations(self, method: str = "nearest"):
        """
        Regrid all observed datasets to their respective model grids.
        ...
        """
        print("\n" + "=" * 80)
        print("REGRIDDING OBSERVATIONS TO MODEL GRIDS")
        print("=" * 80)
        ...
        for idx, config in enumerate(self.obs_datasets):
            ...
            # Perform regridding
            config['obs_regridded'] = self._regrid_data(
                config['obs_ds'],
                config['model_ds'],
                config['obs_varname'],
                config['obs_lat'],
            ...
            print(f"  Regridded shape: {config['obs_regridded'][config['obs_varname']].shape}")
    ...
    def _regrid_data(
        ...
        """
        Regrid observed data to model grid using spatial interpolation.
        """
        print(f"  Using {method} neighbor regridding...")
        
        # Get coordinate arrays
        obs_lat_data = obs_ds[obs_lat].values
        obs_lon_data = obs_ds[obs_lon].values
        model_lat_data = model_ds[model_lat].values
        model_lon_data = model_ds[model_lon].values
        
        # Handle different coordinate shapes
        if obs_lat_data.ndim == 1 and obs_lon_data.ndim == 1:
            obs_lon_grid, obs_lat_grid = np.meshgrid(obs_lon_data, obs_lat_data)
        else:
            obs_lat_grid = obs_lat_data
            obs_lon_grid = obs_lon_data
            
        if model_lat_data.ndim == 1 and model_lon_data.ndim == 1:
            model_lon_grid, model_lat_grid = np.meshgrid(model_lon_data, model_lat_data)
        else:
            model_lat_grid = model_lat_data
            model_lon_grid = model_lon_data
        
        # Flatten coordinates
        obs_points = np.column_stack([
            obs_lat_grid.flatten(),
            obs_lon_grid.flatten()
        ])
        model_points = np.column_stack([
            model_lat_grid.flatten(),
            model_lon_grid.flatten()
        ])
        
        # Build KD-tree for nearest neighbor search
        tree = cKDTree(obs_points)
        distances, indices = tree.query(model_points, k=1 if method == "nearest" else 4)
        
        # Get time dimension
        time_dim = None
        for dim in obs_ds[varname].dims:
            if 'time' in dim.lower():
                time_dim = dim
                break
        ...
        obs_data = obs_ds[varname].values
        n_times = obs_data.shape[0]
        regridded_shape = (n_times,) + model_lat_grid.shape
        ...
        for t in range(n_times):
            ...
            obs_flat = obs_data[t].flatten()
            
            if method == "nearest":
                model_flat = obs_flat[indices]
            else:  # bilinear
                weights = 1.0 / (distances + 1e-10)
                weights = weights / weights.sum(axis=1, keepdims=True)
                model_flat = np.sum(obs_flat[indices] * weights, axis=1)
            
            regridded_data[t] = model_flat.reshape(model_lat_grid.shape)
        
        # Create regridded dataset
        regridded_ds = xr.Dataset(
            {varname: ([time_dim, model_lat, model_lon], regridded_data)},
            coords={
                time_dim: obs_ds[time_dim],
                model_lat: model_lat_data,
                model_lon: model_lon_data,
            }
        )
        
        # Copy attributes
        regridded_ds[varname].attrs = obs_ds[varname].attrs
    ...
    def compute_statistics(self):
        ...
        for idx, config in enumerate(self.obs_datasets):
            ...
            # Get data arrays
            obs_data = config['obs_regridded'][obs_var].values
            ...
            # Monthly statistics
            time_coord = list(config['obs_regridded'].dims.keys())[0]
            times = config['obs_regridded'][time_coord].values
    ...
    def create_visualizations(self):
        ...
        for idx, config in enumerate(self.obs_datasets):
            ...
            obs_units = getattr(
                config['obs_ds'][config['obs_varname']], 'units', ''
            )
            
            # 1. Spatial maps
            self._plot_spatial_maps(config, var_output_dir, var_label, obs_units)
            
            # 2. Time series
            self._plot_time_series(config, var_output_dir, var_label, obs_units)
            
            # 3. Statistics plots
            self._plot_statistics(config, var_output_dir, var_label, obs_units)
    ...
    def _plot_spatial_maps(self, config, output_dir, var_label, units):
        ...
        # Time-averaged maps
        obs_mean = config['obs_regridded'][obs_var].mean(dim=list(config['obs_regridded'].dims.keys())[0])
    ...
    def _plot_time_series(self, config, output_dir, var_label, units):
        ...
        obs_ts = config['obs_regridded'][obs_var].mean(dim=[config['model_lat'], config['model_lon']])
        model_ts = config['model_ds'][model_var].mean(dim=[config['model_lat'], config['model_lon']])
    ...
    def _plot_statistics(self, config, output_dir, var_label, units):
        ...
        obs_data = config['obs_regridded'][obs_var].values.flatten()
    ...
    def save_netcdf_outputs(self):
        """Save regridded observations and statistics as NetCDF."""
        ...
        for config in self.obs_datasets:
            ...
            regridded_file = var_output_dir / f"{var_key}_obs_regridded.nc"
            config['obs_regridded'].to_netcdf(regridded_file)
            print(f"  Saved: {regridded_file}")
            
            # Save spatial statistics
            lat_data = config['model_ds'][config['model_lat']].values
            lon_data = config['model_ds'][config['model_lon']].values
            
            if lat_data.ndim == 1:
                coords = {
                    config['model_lat']: lat_data,
                    config['model_lon']: lon_data
                }
            else:
                coords = {
                    config['model_lat']: ([config['model_lat'], config['model_lon']], lat_data),
                    config['model_lon']: ([config['model_lat'], config['model_lon']], lon_data)
                }
            
            stats_ds = xr.Dataset(
                {
                    'bias': ([config['model_lat'], config['model_lon']], config['spatial_bias']),
                    'rmse': ([config['model_lat'], config['model_lon']], config['spatial_rmse']),
                    'correlation': ([config['model_lat'], config['model_lon']], config['spatial_corr']),
                },
                coords=coords
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Statistical analysis with multiple files per variable using wildcards",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Single variable with wildcards
  python multi_variable_analysis.py \\
      --obs-folder ./obs_data \\
      --obs-pattern "sst_*.nc" \\
      --model-folder ./model_data \\
      --model-pattern "model_sst_*.nc" \\
      --variable sst
  
  # Multiple specific files (space-separated)
  python multi_variable_analysis.py \\
      --obs-folder ./obs \\
  ...
  python multi_variable_analysis.py \\
      ...
      --model-folder ./model ./model \\
      --model-pattern "model_sst_*.nc" "model_sal_*.nc" \\
    ...
    parser.add_argument(
        '--regrid-method', default='nearest', choices=['nearest', 'bilinear'],
